...
class CompaniesControllerTest < ApplicationSystemTestCase
	...
	test "Create Failed" do
		...
		assert_not_equal("new_test_company@test.com", last_company.email)
	end

	test "City And State Test" do
		visit new_company_path
		...
		within("form#new_company") do
			...
			fill_in("company_phone", with: "5553335555")
			fill_in("company_email", with: "new_company@getmainstreet.com")
			click_button "Create Company"
		end
		...
		assert_equal "Georgia", last_company.state
	end

	test "delete" do
		company = Company.where(:status => Company::Status::ACTIVE).last
		delete company_path(company)

		assert_text "Company Successfully Deleted"

		company.reload
		assert_equal Company::Status::DELETED, company.status
	end
end
